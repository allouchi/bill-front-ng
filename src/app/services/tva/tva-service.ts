...
export class TvaService implements ITvaService {
  ...
  private readonly EXERCISE_PATH: string = `${this.apiURL}` + "/tvas/exerciceRef";
  ...
  createOrUpdateTva(tva: Tva): Observable<Tva> {
    ...
    if (isNew) {
      return this.http.post<Tva>(
        this.TVA_PATH,
        tva
      );
    } else {
      return this.http.put<Tva>(
        this.TVA_PATH,
        tva
      );
    }
  ...
  findByExercise(siret: string, exercise: string): Observable<Tva[]> {
    return this.http.get<Tva[]>(
      `${this.TVA_PATH}/${siret}/${exercise}`
    );
  }

  findTvaInfoByExercise(
    siret: string,
    exercise: string
  ): Observable<TvaInfos> {
